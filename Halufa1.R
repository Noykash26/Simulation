## ------------------------------------ Alternative 1 ------------------------------------ ##
#בחרנו במפלגת שמאל
# בחרנו להזיז 2 נציגים מאזור מרכז אל אזור הדרום (ללא תשלום)
# קנינו נציג נוסף לאזור צפון ב5000 שח
# הוספנו דוכן לאזור מרכז משום שלאחר בדיקה, רוב האנשים מגיעים לשם
...
opinion_with_Z<-function (type, Z, area){
  opinion <- c(0.25, 0.5 , 0.25) # initial opinion
  if (area == 2)
    opinion <- c(0.35, 0.45 , 0.20) # initial opinion if the voter is in the center area
  for (i in 1 : 3) {
    if (i == type)
...
north_agents <- c(6, 5, 4) # number of agents at each stand of each kalpi of each area
south_agents <- c(5, 5, 5)
...
for (i in 1:9){ # center stages
  for (j in 1:3) 
    add_resource(elections, paste0("stand_2", i, j), capacity = 8, queue_size = 0)
}
...
traj_initialize <- trajectory("traj_initialize")%>%
  ...
  set_attribute(keys = c("opinion_1", "opinion_2", "opinion_3"), values = function () opinion_with_Z (get_attribute(elections, key = "type"), get_attribute(elections, key = "Z"), get_attribute(elections, key = "area")))%>%
  set_attribute(keys = c("new_opinion_1", "new_opinion_2", "new_opinion_3"), values = function () opinion_with_Z (get_attribute(elections, key = "type"), get_attribute(elections, key = "Z"),get_attribute(elections, key = "area")))%>%
...
measures_halufa1 <- cbind(abandon_measure, rejected_measure, votes_measure)
write.csv(measures_halufa1, "C:\\Users\\User\\Desktop\\project\\halufa1.csv", row.names = TRUE)
