...
# ------------------------- end of simulation- THE RESULTS TATADAM ------------------------------------------
party_1 <- sqldf("select max(value) from full_attribute_data where replication = 1 and key = 'party_1'")
party_2 <- sqldf("select max(value) from full_attribute_data where replication = 1 and key = 'party_2'")
party_3 <- sqldf("select max(value) from full_attribute_data where replication = 1 and key = 'party_3'")
paste0("Left party with ", party_1, " votes!")
paste0("Center party with ", party_2, " votes!")
paste0("Right party with ", party_3, " votes!")
...
reject_left <- sqldf("select replication, count(value) as rejected
                   from full_attribute_data
                   where key = 'reject' and value = 1
                   group by replication")
...
measures_present <- cbind(abandon_measure, rejected_measure, votes_measure)
write.csv(measures_present, "C:\\Users\\User\\Desktop\\project\\present.csv", row.names = TRUE)
